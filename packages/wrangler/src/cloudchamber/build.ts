...
export async function constructBuildCommand(options: {
	...
	console.log("passed in:", options.buildCommand);
...
export async function dockerLoginManagedRegistry(pathToDocker?: string) {
	...
	try {
		await ImageRegistriesService.generateImageRegistryCredentials(domain, {
			...
			child.stdin.end();
			// TODO CDR: write to wrangler log with output unless verbose flag added.
			/*			child.stdout.on("data", (chunk: string) => {
...
export async function pushCommand(
	...
	// Do docker login for managed registry
	// maybe trigger serverless registry automagic if we get there
	// TODO:
	//  Cleanup code
